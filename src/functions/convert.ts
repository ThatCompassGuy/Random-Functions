...
export default function convert(num: number | string, options: Options) : string {
    ...
    switch (from.toLowerCase()) {
        case 'celsius':
            return fromCelsius(num, to.toLowerCase(), round);

        case 'fahrenheit':
            return fromFahrenheit(num, to.toLowerCase(), round);

        case 'kelvin':
            return fromKelvin(num, to.toLowerCase(), round);
